...
namespace unitedata {
    ...
    struct contract_info {
        ...
        account_name admin;
        uint8_t pay_strategy;      // 1 需求方支付固定udt，服务方平均分。2, 提供方获取固定的udt。其他方式待定。
        asset price;
...
//        uint8_t finshed_providers_count;
//        uint8_t canceled_providers_count;
...
//        uint64_t order_id;  // scope 支付的时候作为索引
    ...
    public:
        ...
        void setsigning(uint8_t setsigning);
        ...
        // order
        void createorder(account_name creator, uint64_t order_id, string param, vector<account_name>& providers);    // 创建订单，进入服务方参与阶段
        
        void commitorder(account_name prov, uint64_t order_id, string result);  // 服务方计算完成，提交计算结果
        
        void finishorder(account_name creator, uint64_t order_id, account_name prov);  // 需求方完成订单
        
        void cancelorder(account_name creator, uint64_t order_id, account_name prov);  // 需求方取消订单
        
        void claimreward(account_name prov, uint64_t order_id);  // 需求方取消订单
